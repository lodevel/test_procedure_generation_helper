#!/usr/bin/env python
"""Add CLI argument processing to MainWindow."""
from pathlib import Path

# Read the file
file_path = Path("workflow_editor/main_window.py")
content = file_path.read_text()

# Find the insertion point
marker = "        # Disable tabs and dock until a test is loaded\n        self.tab_widget.setEnabled(False)\n        self.dock.setEnabled(False)\n    \n    def _init_llm_backend(self):"

new_methods = '''        # Disable tabs and dock until a test is loaded
        self.tab_widget.setEnabled(False)
        self.dock.setEnabled(False)
...
if marker in content:
    new_content = content.replace(marker, new_methods)
    file_path.write_text(new_content)
    print("✅ Successfully added showEvent and _process_cli_arguments methods to MainWindow")
else:
    print("❌ Could not find insertion point in main_window.py")
    print("\nLooking for marker ending...")
    if "_init_llm_backend(self):" in content:
        print("✓ Found _init_llm_backend method")
    if "self.dock.setEnabled(False)" in content:
        print("✓ Found dock.setEnabled(False)")
